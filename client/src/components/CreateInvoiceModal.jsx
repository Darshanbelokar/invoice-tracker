...
export default function CreateInvoiceModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    addInvoice({ client: client?.name || form.client, email: client?.email || '', amount: total, due: form.due, status: 'pending', issued: new Date().toISOString().split('T')[0], items: items.length });
    onClose();
    setForm({ client: '', due: '', notes: '', tax: 10 });
    setItems([{ ...emptyItem }]);
  };
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addItem} className="text-xs text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
              ...
              <div key={i} className="grid grid-cols-12 gap-2 px-3 py-2 border-b border-slate-100 last:border-0 items-center">
                <div className="col-span-6">
                  <input placeholder="Item description..." value={item.description}
                    ...
                    className="w-full text-sm border-0 outline-none bg-transparent text-slate-900 placeholder-slate-400" />
                </div>
                <div className="col-span-2">
                  <input type="number" min="1" value={item.qty}
                    onChange={e => updateItem(i, 'qty', e.target.value)}
                    className="w-full text-sm text-center border border-slate-200 rounded-lg px-2 py-1 bg-white outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div className="col-span-2">
                ...
                <div className="col-span-2 flex items-center justify-end gap-1">
                  ...
                  {items.length > 1 && (
                    <button type="button" onClick={() => removeItem(i)} className="text-slate-300 hover:text-red-500 ml-1">
          ...
          <div className="bg-slate-50 rounded-xl p-4 space-y-2">
            ...
            <div className="flex justify-between text-sm text-slate-600 items-center">
              ...
              <input type="number" value={form.tax} onChange={e => setForm({ ...form, tax: e.target.value })}
                className="w-20 text-sm text-right border border-slate-200 rounded-lg px-2 py-1 bg-white outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="flex justify-between text-sm text-slate-600">
